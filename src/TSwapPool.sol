...
contract TSwapPool is ERC20 {
    ...
    constructor(
        ...
        // @written lack zero address check
    ...
    // q if it's empty, how does it know what to do?
    function deposit(
        uint256 wethToDeposit,
        uint256 minimumLiquidityTokensToMint,
        uint256 maximumPoolTokensToDeposit,
        // @written deadline is not being used
        // if someone sets a deadline, let's say next block
        // they then could still deposit.
        // A user who expectes a deposit to fail, will go through.
        ...
        returns (uint256 liquidityTokensToMint)
    {
        if (wethToDeposit < MINIMUM_WETH_LIQUIDITY) {
            // @written MINIMUM_WETH_LIQUIDITY is a constant and therefore not required to be emitted.
        ...
        if (totalLiquidityTokenSupply() > 0) {
            ...
            // e this is not state variable
            // @written it would be better if this was before the `_addLiquidityMintAndTransfer` function call to follow CEI
    ...
    function _addLiquidityMintAndTransfer(
        ...
        // @written this is backwards
        // should be: emit LiquidityAdded(msg.sender, wethToDeposit, poolTokensToDeposit);
        ...
        returns (uint256 inputAmount)
    {
        ...
        // @written users are charged way tooooo much
        return
            ((inputReserves * outputAmount) * 10000) /
            ((outputReserves - outputAmount) * 997);
    }

    // @written where is the netspec?
    ...
    // @written missing deadline params in natspec
        ...
        returns (uint256 inputAmount)
    {
        ...
        // @written need a max input amount, no slippage protection
        // MEV attack
